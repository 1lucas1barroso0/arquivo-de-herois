...
import { getDerivedTraits } from "../lib/rules";
import {
...
function ComparisonPanel({
  ...
  async function select(id: string) {
    ...
    try {
      ...
      setComparison(normalizeSheet(payload.character));
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <section className="sheet-tool-page comparison-page">
      <header>
        <div>
          <p className="eyebrow">{m("character.compare")}</p>
          <h1>{sheet.heroName}</h1>
          <p>{m("compare.description")}</p>
        </div>
        ...
        >
          <option value="">{m("compare.choose")}</option>
          {characters
      ...
      {loading ? (
        ...
        <ComparisonTable left={sheet} right={comparison} />
      ) : (
        <p className="inline-empty">
          <ArrowDownUp /> {m("compare.empty")}
        </p>
      )}
    </section>
  );
...
function ComparisonTable({
  ...
  const rows: Array<[string, string | number, string | number]> = [
    ...
    [
      "PP",
      `${a.points.spent}/${a.points.budget}`,
      `${b.points.spent}/${b.points.budget}`,
    ],
